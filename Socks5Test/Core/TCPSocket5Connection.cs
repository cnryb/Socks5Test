...
namespace Socks5Test.Core
{
    ...
    public class TCPSocks5Connection
    {
        ...
        /// 服务端
        /// </summary>
        private TCPSocks5Server Server { get; set; }

        /// <summary>
        /// 客户端
        /// </summary>
        internal Socket Client { private set; get; }
        ...
        {
            if (!this.Server.IsStarting) return;
        ...
        private bool ValidateIdentity()
        {
            ...
            this.Server.Logger.WriteLine(string.Format("于{0}接收到客户端{1}的身份验证请求[UserName={3},Password={4}],结果:{2}...",
        ...
        private bool DoProtocolRequest()
        {
            ...
            if (SocketUtils.Receive(this.Client, 4, out byte[] buffer))
            {
                if (buffer.Length == 4)
                {
                    //判断地址类型
                        ...
                        case 0x03:
                            ...
                            if (SocketUtils.Receive(this.Client, 1, out buffer))
                            {
                                ...
                                if (SocketUtils.Receive(this.Client, (uint)(buffer[0]), out buffer))
                                {
                                    ...
                                    {
                                        rep = 0x04;  //主机不可达
                                        Server.Logger.WriteLine($"域名无法解析 {domainName} \t {ex.Message}");
                                    }
                                }
                            }
                            break;
            ...
            if (ipAddress != null && rep == 0x07)
            {
                ...
                if (SocketUtils.Receive(this.Client, 2, out buffer))
                {
                    ...
                    rep = 0x00;
                    this.Server.Logger.WriteLine(string.Format("于{0}接收到客户端要求对主机{1}进行连接的请求....", DateTime.Now, this.RemoteEndPoint));
                }
            }
        ...
        private void OnClientReceive(IAsyncResult result)
        {
            ...
            try
            {
                ...
                int size = this.Client.EndReceive(result, out SocketError error);
                if (size > 0)
                {
                    //转发给代理
        ...
        private void OnProxyReceive(IAsyncResult result)
        {
            ...
            {
                Socket socket = result.AsyncState as Socket;
                int size = socket.EndReceive(result, out SocketError error);
                if (size > 0)
                {
                    //转发给客户端
        ...
        private void Close()
        {
            try
            {
                if (this.Client != null)
                {
                    this.Server.Logger.WriteLine(string.Format("已于{0}关闭与客户端{1}的连接...", DateTime.Now, this.Client.RemoteEndPoint));
                    this.Client.Close(3);
                    this.Client = null;
                }
                if (this.Proxy != null)
                {
                    this.Server.Logger.WriteLine(string.Format("代理端已于{0}关闭与远程服务器{1}的连接...", DateTime.Now, this.Proxy.Client.RemoteEndPoint));
                    this.Proxy.Close();
                    this.Proxy = null;
                }
            }
            ...
            {
                this.Server.Logger.WriteLine(ex.Message);
            }
